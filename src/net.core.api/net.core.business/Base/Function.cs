...
namespace net.core.business.Base;

public static class Function
    {
        #region --- Cut Text ---
        public static string CutText(string Value, int CharacterLimit)
        {
            if (!String.IsNullOrEmpty(Value))
            {
                int TotalChar = Value.Length;
                if (CharacterLimit <= TotalChar)
                {
                    for (int i = 1; i < TotalChar; i++)
                    {
                        if (CharacterLimit <= i)
                        {
                            string Spacer = Value.Substring(i, 1);
                            if (Spacer == " ") return Value.Substring(0, i) + "...";
                        }
                    }
                    return "";
                }
        ...
        public static string Substring(this object Value, int CharacterLimit, SubstringType Type = SubstringType.Space)
        {
            if (Value != null)
            {
                string NewValue = Value.ToString();
                int TotalChar = NewValue.Length;

                if (CharacterLimit <= TotalChar)
                {
                    if (Type == SubstringType.Space)
                    {
                        for (int i = 1; i < TotalChar; i++)
                        {
                            if (CharacterLimit <= i)
                            {
                                string Spacer = NewValue.Substring(i, 1);
                                if (Spacer == " ") return NewValue.Substring(0, i) + "...";
                            }
                        }
                        return NewValue;
                    }
                    ...
                    {
                        return NewValue.Substring(0, CharacterLimit) + "...";
                    }
                }
                else
                {
        ...
        #region --- Cut a Long Words --- | Result: ooooo ooooo oo
        public static string CutWord(string Word, int Number)
        {
            string NewWord = "";
            string[] Splitter = Word.Split(' ');
            for (int h = 0; h < Splitter.Length; h++)
            {
                string CWord = Splitter[h];
                int WordNum = CWord.Length;
                if (WordNum > Number)
                {
                    int B = 0;
                    for (int k = 0; k < (WordNum / Number); k++)
                    {
                        if (WordNum >= (B + Number))
                        {
                            NewWord = NewWord + CWord.Substring(B, Number) + " ";
                            B += Number;
                        }
                        else
                        {
                            NewWord = NewWord + CWord.Substring(B);
                        }
                    }
                }
                else
                {
                    NewWord = NewWord + CWord + " ";
                }
            }
            return NewWord;
        }
        ...
        #region --- HTML Deactive --- | Result: &lt;font size=3&gt;demo&lt;/font&gt;
        public static string DeactiveHTML(string Value)
        {
            string NewText = Value;
            NewText = NewText.Replace(Convert.ToString((char)13), "<br />");
        ...
        #region --- HTML Cleaner --- | Orginal: "<font>demo</font>, <b>demo</b>" | Result: demo, demo
        ...
        #region --- File Name Editor ---  |  Orginal: Mustafa Kemal Atatürk  |  Result: mustafa_kemal_ataturk
        public static string FileNameEditor(string Value)
        {
            string NewWord = "";
            string[] AllText = "a,b,c,d,e,f,g,h,i,j,k,l,m,n,o,p,r,s,t,u,v,y,z,x,w,q,1,2,3,4,5,6,7,8,9,0,-,_,.,@".Split(',');

            List<string> WordList = new List<string>();
            for (int j = 0; j < AllText.Length; j++)
            {
                WordList.Add(AllText[j]);
            }
            Value = Value.ToLower();
            ...
            {
                string LookWord = Value.Substring(y, 1);
                for (int b = 0; b < WordList.Count; b++)
                {
                    string LisWord = WordList[b];
                    if (LookWord == LisWord)
                    {
        ...
        #region --- Character Shifter ---  |  Orginal: Mustafa Kemal Atatürk  |  Result: mustafa_kemal_ataturk
        public static string CharacterShifter(string Value, string Characters)
        {
            string NewWord = "";
            string[] AllText = Characters.Split('|');
        ...
        #region --- String Limit ---  |  Orginal: Mustafa Kemal Atatürk  |  Result: TAYYIPERDOGAN
        public static string StringLimit(string Value)
        {
            string NewWord = "";
            string[] AllText = "A,B,C,D,E,F,G,H,I,J,K,L,M,N,O,P,R,S,T,U,V,Y,Z,W,X,Q,1,2,3,4,5,6,7,8,9,0".Split(',');
        ...
        #region --- Bracket Cleaner ---   |   Orginal: "Mustafa Kemal Atatürk'ün"  |  Result: &quot;Mustafa Kemal Atatürk’ün&quot;
        public static string BracketCleaner(string Value)
        {
            string NewValue = Value;
            //NewValue = NewValue.Replace(Convert.ToString((char)34), "&quot;");
            NewValue = NewValue.Replace("'", "''");
            return NewValue;
        }
        #endregion

        #region --- Clear Text ---   |   Orginal: "<font size="10">Mustafa Kemal Atatürk'ün</font>"  |  Result: &quot;Mustafa Kemal Atatürk’ün&quot;
        public static string ClearText(string Value)
        {
            string NewValue = Value;
            NewValue = BracketCleaner(NewValue);
            NewValue = CleanHTML(NewValue);
            return NewValue;
        }
        #endregion

        #region --- Randomize Integer ---
        public static int RandomNumber(int Min, int Max, int Count)
        {
            string TotalNumber = "";
            Random NumberCreate = new Random();
            for (int i = 0; i < Count; i++)
            {
                TotalNumber += NumberCreate.Next(Min, Max).ToString();
            }
            return Convert.ToInt32(TotalNumber);
        }
        #endregion

        #region --- Is Decimal Control ---
        ...
        #region --- Is Integer Control ---
        public static bool IsInteger(object Text)
        {
            if (Text != null)
            {
                int result = int.TryParse(Text.ToString(), out result) ? result : -1;
        ...
        #region --- Number Complement ---   |   Orginal: 10   |   Result: 0000010
        public static string NumberComplement(string SetNumber, int SetZero)
        {
            string TotalNumber = "";
            for (int c = 0; c < (SetZero - SetNumber.Length); c++)
            {
                TotalNumber += "0";
            }
            TotalNumber += SetNumber;
            return TotalNumber;
        }
        #endregion

        #region --- Number Point Changer ---    |   Orginal: 10.000,25   |   Result: 10000.25
        public static string NumberPointChanger(string Number)
        {
        ...
        #region --- URL ReWrite ---  |  Orginal: Mustafa Kemal Atatürk  |  Result: mustafa-kemal-ataturk
        public static string ReWrite(this string Text)
        {
            string NewWord = "";
            if (!String.IsNullOrEmpty(Text))
            {
                ...
                {
                    string LookWord = Text.Substring(y, 1);
                    for (int b = 0; b < WordList.Count; b++)
                    {
                        string LisWord = WordList[b];
                        if (LookWord == LisWord)
        ...
        #region --- To Title Case ---   |   Orginal: bu bir denemedir   Result: Bu Bir Denemedir
        ...
        #region --- Clear Turkish Characters ---  |  Orginal: Mustafa Kemal Atatürk  |  Result: Mustafa Kemal Ataturk
        public static string ClearTurkishCharacter(this object Value, params object[] args)
        {
            string result = "";
            if (Value != null)
            {
        ...
        #region --- MimeTypes ---   |   Orginal: jpg   |   Result: image/jpeg
        ...
        #region --- To First Lower Case ---   |   Orginal: bu bir denemedir   Result: bu Bir Denemedir
        public static string ToFirstLowerCase(this string Text)
        {
            return FirstLowerCase(Text, null);
        }

        public static string ToFirstLowerCase(this string Text, CultureInfo ci)
        {
            return FirstLowerCase(Text, ci);
        }

        private static string FirstLowerCase(string Text, CultureInfo ci)
        {
            if (ci == null) ci = CultureInfo.CurrentCulture;
            //Text = ci.TextInfo.ToTitleCase(Text);
            Text = Text.Substring(0, 1).ToLower(ci) + Text.Substring(1);
            return Text;
        }
        #endregion

        #region --- Column Name Editor ---  |  Orginal: Mustafa Kemal Atatürk  |  Result: MustafaKemalAtaturk
        public static string ColumnNameEditor(string value)
        {
            string result = "";
            ...
            if (!String.IsNullOrEmpty(value))
            {
                ...
                string[] AllText = "a,b,c,d,e,f,g,h,i,j,k,l,m,n,o,p,r,s,t,u,v,y,z,x,w,q,A,B,C,D,E,F,G,H,I,J,K,L,M,N,O,P,R,S,T,U,V,Y,Z,W,Q,X,1,2,3,4,5,6,7,8,9,0".Split(',');

                List<string> charList = new List<string>();
                for (int j = 0; j < AllText.Length; j++)
                {
                    charList.Add(AllText[j]);
                }

                for (int y = 0; y < value.Length; y++)
                {
                    string lookChar = value.Substring(y, 1);
                    for (int b = 0; b < charList.Count; b++)
                    {
                        string lisChar = charList[b];
                        if (lookChar == lisChar)
                        {
                            result = result + lookChar;
                        }
                    }
                }
